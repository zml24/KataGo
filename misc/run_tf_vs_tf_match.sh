...
usage() {
  cat <<'EOF'
用法:
  run_tf_vs_tf_match.sh KATAGO_BIN TF_MODEL_A TF_MODEL_B OUTPUT_DIR [NUM_GAMES] [MAX_VISITS]

参数:
  KATAGO_BIN   katago 可执行文件
  TF_MODEL_A   Transformer 模型 A 路径，例如 .kgtr.gz
  TF_MODEL_B   Transformer 模型 B 路径，例如 .kgtr.gz
  OUTPUT_DIR   输出目录
  NUM_GAMES    可选，默认 500
  MAX_VISITS   可选，默认 500

环境变量:
  TF_NAME_A                      默认取 TF_MODEL_A 文件名
  TF_NAME_B                      默认取 TF_MODEL_B 文件名
  TF_PRECISION_A                 A 精度，默认 fp16，可选 fp32/fp16/bf16
  TF_PRECISION_B                 B 精度，默认 fp16，可选 fp32/fp16/bf16
...
示例:
  TF_PRECISION_A=fp32 TF_PRECISION_B=bf16 \
  NUM_GAME_THREADS=16 \
  ./misc/run_tf_vs_tf_match.sh \
    /path/to/katago \
    /path/to/model_a.kgtr.gz \
    /path/to/model_b.kgtr.gz \
    /tmp/tf-vs-tf \
    400 \
    800
EOF
...
TF_NAME_A=${TF_NAME_A:-$(basename "$TF_MODEL_A")}
TF_NAME_B=${TF_NAME_B:-$(basename "$TF_MODEL_B")}
TF_PRECISION_A=${TF_PRECISION_A:-fp16}
TF_PRECISION_B=${TF_PRECISION_B:-fp16}
...
if [[ ! -f "$TF_MODEL_A" ]]; then
  echo "TF_MODEL_A 不存在: $TF_MODEL_A" >&2
  exit 1
fi
if [[ ! -f "$TF_MODEL_B" ]]; then
  echo "TF_MODEL_B 不存在: $TF_MODEL_B" >&2
  exit 1
fi
...
for prec_var in TF_PRECISION_A TF_PRECISION_B; do
  prec_val="${!prec_var}"
  case "$prec_val" in
    fp32|fp16|bf16) ;;
    *)
      echo "$prec_var 必须是 fp32 / fp16 / bf16，当前为: $prec_val" >&2
      exit 1
      ;;
  esac
done

if [[ "$TF_NAME_A" == "$TF_NAME_B" ]]; then
  TF_NAME_A="${TF_NAME_A}[${TF_PRECISION_A}]"
  TF_NAME_B="${TF_NAME_B}[${TF_PRECISION_B}]"
fi
...
CONFIG_FILE="$OUTPUT_DIR/match_tf_vs_tf.cfg"
...
nnModelFile0 = $TF_MODEL_A
nnModelFile1 = $TF_MODEL_B
nnModelType0 = tf
nnModelType1 = tf

nnPrecision-0 = $TF_PRECISION_A
nnPrecision-1 = $TF_PRECISION_B
...
if compgen -G "$SGF_DIR/*.sgfs" > /dev/null || compgen -G "$SGF_DIR/*.sgf" > /dev/null; then
  python3 - "$SGF_DIR" "$TF_NAME_A" "$TF_NAME_B" "$SUMMARY_FILE" <<'PY'
import math
import re
import sys
...
for path in paths:
    ...
    for record in split_sgf_records(text):
        ...
        if outcome == "black":
            ...
            b_scores.append(1.0 if winner == name_b else 0.0)
        elif outcome == "white":
            winner = white_player
            loser = black_player
            stats[winner]["wins"] += 1
...
if b_scores:
    ...
    b_rate = b_score / n
    b_elo = rate_to_elo(b_rate)
    lines.append(f"{name_b} 相对 {name_a} 记分胜率: {b_score:.1f}/{n} = {b_rate:.4f}")
    lines.append(f"{name_b} 相对 {name_a} Elo 估计: {b_elo:.1f}")
    if n > 1:
        mean = b_rate
        variance = sum((x - mean) ** 2 for x in b_scores) / (n - 1)
        ...
        lines.append(f"{name_b} 相对 {name_a} 记分胜率 95% CI: [{ci_low:.4f}, {ci_high:.4f}]")
        lines.append(f"{name_b} 相对 {name_a} Elo 95% CI: [{elo_low:.1f}, {elo_high:.1f}]")
else:
    lines.append(f"{name_b} 相对 {name_a} Elo 估计: N/A（没有可用于 Elo 的有效对局）")
