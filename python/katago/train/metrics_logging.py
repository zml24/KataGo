...
def log_metrics(metric_sums, metric_weights, metrics, metrics_out, tb_writer=None, tb_global_step=None, tb_prefix="train"):
    metrics_to_print = {}
    for metric in metric_sums:
        if metric.endswith("_sum"):
    ...
    if tb_writer is not None and tb_global_step is not None:
        for metric, value in metrics_to_print.items():
            if isinstance(value, (int, float, np.floating, np.integer)) and math.isfinite(value):
